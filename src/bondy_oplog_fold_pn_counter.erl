...
-module(bondy_oplog_fold_pn_counter).
...
?MODULEDOC("""
Positive-Negative Counter (PN-Counter) fold.

An integer counter that supports concurrent `inc` and `dec` operations
across replicas without coordination. Per-Origin accumulation of
positive and negative contributions; the projected value is
`sum(Pos) - sum(Neg)` across all observed Origins.
...
Each Origin contributes a `{Pos, Neg, MaxSeq}` triple. `MaxSeq` is the
...
`Delta` may be negative — a "decrement" is just `{inc, -K}`. Origin
and Seq come from the WAL event key via `Meta`; they are not embedded
in the payload.

## Idempotency and ordering

Idempotency rests on the per-Origin `MaxSeq` dedup. Replay or
duplicate sync delivery is a no-op because the per-Origin Seq monotonic
counter never reuses a value (see `_design/0_architecture.md` §4.2).

`Pos` / `Neg` accumulation correctness relies on the substrate's
...
within a cell. Per-Origin max-merge over `{Pos, Neg, MaxSeq}` is then
correct because the higher MaxSeq always reflects a strictly larger
prefix of the same Origin's contiguous event stream.

## Merge

Per-Origin element-wise max across `{Pos, Neg, MaxSeq}`. Naturally
...
counter entry -> <<OriginSize:16, Origin/binary,
                   Pos:64, Neg:64, MaxSeq:64>>

event (inc) -> <<1, Delta:64/signed>>
```

The encoding is canonical: counter entries are sorted by Origin so two
...
apply_event(#{counters := C0, hlc := H0} = S, {inc, Delta}, Meta)
        when is_integer(Delta), Meta =/= undefined ->
...
to_value(#{counters := C}) ->
    maps:fold(
        fun(_O, {P, N, _S}, Acc) -> Acc + P - N end,
        0,
        C).


-spec apply_value_delta(integer(), integer()) -> integer().

apply_value_delta(Value, Delta) when is_integer(Value), is_integer(Delta) ->
    Value + Delta.


-spec merge_states(state(), state()) -> state().

merge_states(#{counters := Ca, hlc := Ha},
             #{counters := Cb, hlc := Hb}) ->
    Origins = lists:usort(maps:keys(Ca) ++ maps:keys(Cb)),
    Merged = lists:foldl(
        fun(O, Acc) ->
            {PA, NA, SA} = maps:get(O, Ca, {0, 0, 0}),
            {PB, NB, SB} = maps:get(O, Cb, {0, 0, 0}),
            Acc#{O => {erlang:max(PA, PB),
                       erlang:max(NA, NB),
                       erlang:max(SA, SB)}}
        end,
        #{},
...
decode_state(<<H:64/big-unsigned,
               NumOrigins:32/big-unsigned, Rest0/binary>>) ->
...
encode_event({inc, Delta}) when is_integer(Delta) ->
    <<1, Delta:64/big-signed>>.


-spec decode_event(binary()) -> event().

decode_event(<<1, Delta:64/big-signed>>) ->
    {inc, Delta}.
...
encode_entry(Origin, {Pos, Neg, MaxSeq})
        when is_binary(Origin),
             is_integer(Pos), Pos >= 0,
             is_integer(Neg), Neg >= 0,
             is_integer(MaxSeq), MaxSeq >= 0 ->
    OriginSize = byte_size(Origin),
    <<OriginSize:16/big-unsigned, Origin/binary,
      Pos:64/big-unsigned,
      Neg:64/big-unsigned,
...
decode_entries(N, <<OriginSize:16/big-unsigned, Origin:OriginSize/binary,
                    Pos:64/big-unsigned,
                    Neg:64/big-unsigned,
                    MaxSeq:64/big-unsigned, Rest/binary>>, Acc) when N > 0 ->
