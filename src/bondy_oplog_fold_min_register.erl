...
-module(bondy_oplog_fold_min_register).
...
?MODULEDOC("""
Min-Register fold — an integer register whose value is the **minimum**
of every value ever written.

Mirror of `bondy_oplog_fold_max_register`: same shape and semantics,
but the value lattice is min-merge instead of max-merge. The HLC field
is still merged by `max/2` (it carries last-modified metadata, not
value semantics).

To "reset" a Min-Register, allocate a fresh key — there is no
clear/revoke.
...
See `bondy_oplog_fold_max_register` for the design notes — every
property carries over with `max` replaced by `min` on the value field.
""").
