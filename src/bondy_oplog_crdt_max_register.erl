...
-module(bondy_oplog_crdt_max_register).
...
?MODULEDOC("""
Max-Register — native operation-based CRDT.

The op-based twin of the deprecated `bondy_oplog_fold_max_register`, with
**identical state, encoding, and semantics** — expressed as a single
operation step (`apply_op/3`), no `merge_states`, no value delta.

An integer register whose value is the **maximum** of every value ever
written. `max` is commutative, associative and idempotent, so this type is
order-independent over all permutations. To "reset", allocate a fresh key.
...
-doc """
Apply one `{set, V}` in key order — keep the larger value and the larger
...
apply_op({Old, OldH}, {set, V}, Key) when is_integer(V) ->
    {erlang:max(Old, V), erlang:max(OldH, bondy_oplog_event:key_hlc(Key))}.
