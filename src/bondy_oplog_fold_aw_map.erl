...
apply_event(#{entries := E, hlc := H0} = S,
            {put, K, SubFold, SubInitState}, Meta)
        when is_binary(K), is_atom(SubFold), Meta =/= undefined ->
    {Dot, H1} = dot_and_hlc(Meta, H0),
    {NewEntry, Delta} = put_into(K, SubFold, SubInitState, Dot,
...
apply_event(#{entries := E, hlc := H0} = S,
            {apply, K, SubFold, SubEvent}, Meta)
        when is_binary(K), is_atom(SubFold), Meta =/= undefined ->
    {Dot, H1} = dot_and_hlc(Meta, H0),
    {NewEntry, Delta} = apply_into(K, SubFold, SubEvent, Dot, Meta,
                                   maps:get(K, E, not_found)),
    case NewEntry of
        keep ->
            {S#{hlc := H1}, none};
...
apply_event(#{entries := E, hlc := H0} = S,
            {remove, K, ObservedDots}, Meta)
        when is_binary(K), is_list(ObservedDots), Meta =/= undefined ->
    ...
    case maps:get(K, E, not_found) of
        ...
        {tombstoned, SubFold, Tombs} ->
            NewTombs = ordsets:union(Tombs, DotsSet),
            {S#{entries := E#{K => {tombstoned, SubFold, NewTombs}},
                hlc := H1},
             none}
    end.
...
to_value(#{entries := E}) ->
    maps:fold(
        fun(K, {F, SS, _AddDots, _Tombs}, Acc) ->
...
- `{remove_elem, K}` — K is no longer live.
""".
-spec apply_value_delta(map(),
                        {set_elem, key_v(), term()}
                        | {remove_elem, key_v()}) -> map().
...
merge_states(#{entries := Ea, hlc := Ha},
             #{entries := Eb, hlc := Hb}) ->
    Keys = lists:usort(maps:keys(Ea) ++ maps:keys(Eb)),
    Merged = lists:foldl(
        fun(K, Acc) ->
            Acc#{K => merge_entries(K,
...
gc_threshold(#{entries := E, hlc := H}) ->
    maps:fold(
        fun(_K, {F, SS, _AddDots, _Tombs}, Acc) ->
...
decode_state(<<H:64/big-unsigned,
               N:32/big-unsigned, Rest0/binary>>) ->
...
encode_event({put, K, SubFold, SubInitState})
        when is_binary(K), is_atom(SubFold) ->
    Tag = bondy_oplog_fold:tag_of(SubFold),
    SubBin = bondy_oplog_fold:encode_state(SubFold, SubInitState),
    <<1,
      (byte_size(K)):32/big-unsigned, K/binary,
      Tag:8,
      (byte_size(SubBin)):32/big-unsigned, SubBin/binary>>;

encode_event({apply, K, SubFold, SubEvent})
        when is_binary(K), is_atom(SubFold) ->
    Tag = bondy_oplog_fold:tag_of(SubFold),
...
encode_event({remove, K, ObservedDots})
        when is_binary(K), is_list(ObservedDots) ->
...
decode_event(<<1, KSize:32/big-unsigned, K:KSize/binary,
               Tag:8,
               SubSize:32/big-unsigned, Sub:SubSize/binary>>) ->
    SubFold = bondy_oplog_fold:mod_of_tag(Tag),
    SubInitState = bondy_oplog_fold:decode_state(SubFold, Sub),
    {put, K, SubFold, SubInitState};

decode_event(<<2, KSize:32/big-unsigned, K:KSize/binary,
               Tag:8,
               SubSize:32/big-unsigned, Sub:SubSize/binary>>) ->
    SubFold = bondy_oplog_fold:mod_of_tag(Tag),
...
decode_event(<<3, KSize:32/big-unsigned, K:KSize/binary,
...
dot_and_hlc(Meta, H0) ->
    Dot = {bondy_oplog_event:key_origin(Meta),
           bondy_oplog_event:key_seq(Meta)},
    H1 = erlang:max(H0, bondy_oplog_event:key_hlc(Meta)),
    {Dot, H1}.
...
put_into(K, SubFold, SubInitState, Dot,
    ...
    case ordsets:is_element(Dot, Tombs) of
        true ->
            ...
            %% replay convergence.
            keep_or_update(K, SubFold, SubState0, SubState1,
                           AddDots0, Tombs);
        false ->
            keep_or_update(K, SubFold, SubState0, SubState1,
                           AddDots1, Tombs)
...
apply_into(K, SubFold, SubEvent, Dot, Meta, not_found) ->
    ...
    {SubState1, _} = bondy_oplog_fold:apply_event(SubFold, SubInit, SubEvent, Meta),
    NewEntry = {SubFold, SubState1, [Dot], []},
    Value = bondy_oplog_fold:to_value(SubFold, SubState1),
    {NewEntry, {set_elem, K, Value}};

%% apply on a live key with matching SubFold
apply_into(K, SubFold, SubEvent, Dot, Meta,
           {SubFold, SubState0, AddDots0, Tombs}) ->
    {SubState1, _SubDelta} =
        bondy_oplog_fold:apply_event(SubFold, SubState0, SubEvent, Meta),
    AddDots1 = case ordsets:is_element(Dot, Tombs) of
        true  -> AddDots0;  %% can't re-add a tombstoned observation
...
merge_entries(_K, undefined, B) -> B;
merge_entries(_K, A, undefined) -> A;
...
encode_entry(K, {F, SubState, AddDots, Tombs}) ->
    ...
    <<(byte_size(K)):32/big-unsigned, K/binary,
      1,
      Tag:8,
      (length(AddDots)):32/big-unsigned, AddBin/binary,
      (length(Tombs)):32/big-unsigned, TombBin/binary,
      (byte_size(SubBin)):32/big-unsigned, SubBin/binary>>;
...
encode_dot({Origin, Seq})
        when is_binary(Origin),
             is_integer(Seq), Seq >= 0 ->
    <<(byte_size(Origin)):16/big-unsigned, Origin/binary,
      Seq:64/big-unsigned>>.
...
decode_entries(N, <<KSize:32/big-unsigned, K:KSize/binary,
                    1, Tag:8,
                    NumDots:32/big-unsigned, RestDots/binary>>, Acc)
        when N > 0 ->
...
decode_entries(N, <<KSize:32/big-unsigned, K:KSize/binary,
                    2, Tag:8,
                    NumTombs:32/big-unsigned, RestTombs/binary>>, Acc)
        when N > 0 ->
...
decode_dots(0, Rest, Acc) ->
    {lists:reverse(Acc), Rest};
decode_dots(N, <<OSize:16/big-unsigned, O:OSize/binary,
                 Seq:64/big-unsigned, Rest/binary>>, Acc)
        when N > 0 ->
