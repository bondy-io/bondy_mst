...
The library does **not** implement a hot-backup write-barrier in
this PR — operators wanting zero-downtime backups use a
filesystem-level snapshot (LVM, ZFS, btrfs, EBS) of `storage_path`,
which is then handed to `backup/3` for the manifest + checksum step.
...
own data dir" (see `_design/STORAGE_ARCHITECTURE.md` §13.5).
...
writer is live would be torn. The runbook (`_design/RUNBOOK_BACKUP.md`)
gives the recommended sequence.
""").
