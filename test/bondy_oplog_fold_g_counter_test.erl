%% =============================================================================
%% EUnit smoke tests for `bondy_oplog_fold_g_counter`.
%%
%% Covers `apply_event/3` per-Origin accumulation, MaxSeq dedup,
%% non-negative delta guard, `to_value/1` sum projection,
...
-module(bondy_oplog_fold_g_counter_test).

-include_lib("eunit/include/eunit.hrl").

-define(MOD, bondy_oplog_fold_g_counter).
...
single_inc_records_count_test() ->
    H = hlc(100, 0),
    S = apply_inc(?MOD:initial_value(), 5, <<"a">>, 1, H),
    ?assertEqual(#{<<"a">> => {5, 1}}, maps:get(counters, S)),
    ?assertEqual(H, maps:get(hlc, S)).

zero_inc_is_recorded_test() ->
    %% A zero-delta inc is still an observation: counters get an
    %% entry for the Origin, MaxSeq bumps to the event's Seq.
    H = hlc(100, 0),
    S = apply_inc(?MOD:initial_value(), 0, <<"a">>, 1, H),
    ?assertEqual(#{<<"a">> => {0, 1}}, maps:get(counters, S)),
    ?assertEqual(H, maps:get(hlc, S)).

multi_inc_same_origin_accumulates_test() ->
    S0 = ?MOD:initial_value(),
    S1 = apply_inc(S0, 5, <<"a">>, 1, hlc(100, 0)),
    S2 = apply_inc(S1, 3, <<"a">>, 2, hlc(101, 0)),
    S3 = apply_inc(S2, 7, <<"a">>, 3, hlc(102, 0)),
    ?assertEqual(#{<<"a">> => {15, 3}}, maps:get(counters, S3)),
    ?assertEqual(15, ?MOD:to_value(S3)).

negative_delta_crashes_test() ->
    %% The non-negative guard rejects malformed input loudly. The
    %% applier (`apply_one_cell`) catches and logs.
    H = hlc(100, 0),
    ?assertError(
        function_clause,
        ?MOD:apply_event(?MOD:initial_value(), {inc, -3}, key(H, <<"a">>, 1))
    ).
...
duplicate_seq_is_value_noop_test() ->
    ...
    %% Replay the same Seq at a higher HLC (e.g. crash recovery sees
    %% the same event with a fresh receive-time HLC). Counters
    %% unchanged; cell HLC bumps to track last-observed.
    S1 = apply_inc(S0, 5, <<"a">>, 1, hlc(200, 0)),
    ?assertEqual(#{<<"a">> => {5, 1}}, maps:get(counters, S1)),
...
older_seq_is_rejected_test() ->
    ...
    ?assertEqual(#{<<"a">> => {5, 2}}, maps:get(counters, S1)).
...
two_origins_independent_test() ->
    ...
    ?assertEqual({5, 1}, maps:get(<<"a">>, maps:get(counters, S2))),
    ?assertEqual({7, 1}, maps:get(<<"b">>, maps:get(counters, S2))),
...
to_value_sums_counts_across_origins_test() ->
    S = #{
        counters => #{<<"a">> => {10, 5}, <<"b">> => {5, 3}},
        hlc => hlc(100, 0)
    },
    ?assertEqual(15, ?MOD:to_value(S)).
...
delta_for_duplicate_event_is_none_test() ->
    %% Replay of the same (Origin, Seq) is dedup'd: state HLC bumps
    %% but the value column does not move — `apply_event/3` returns
    %% `none`.
    S0 = apply_inc(?MOD:initial_value(), 5, <<"a">>, 1, hlc(100, 0)),
    ?assertEqual(
        none,
...
apply_value_delta_adds_test() ->
    ...
    ?assertEqual(3, ?MOD:apply_value_delta(3, 0)).
...
merge_distinct_origins_unions_test() ->
    ...
    ?assertEqual({5, 1}, maps:get(<<"a">>, maps:get(counters, M))),
    ?assertEqual({7, 1}, maps:get(<<"b">>, maps:get(counters, M))).

merge_same_origin_max_per_field_test() ->
    %% Per-Origin contiguous prefix: A has 5+3 (Count=8), B has 5+3+2
...
dispatcher_resolves_shorthand_test() ->
    ?assertEqual(?MOD, bondy_oplog_fold:mod_of(g_counter)).

dispatcher_is_known_test() ->
    ?assertEqual(true, bondy_oplog_fold:is_known(g_counter)).

dispatcher_value_equals_state_false_test() ->
    ?assertEqual(false, bondy_oplog_fold:value_equals_state(g_counter)).

dispatcher_apply_value_delta_dispatch_test() ->
    ?assertEqual(9, bondy_oplog_fold:apply_value_delta(g_counter, 5, 4)).
