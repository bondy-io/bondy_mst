%% =============================================================================
%% PropEr properties for `bondy_oplog_fold_map_of_fields`.
%%
...
%%   - prop_gc_safe/0                               (§5.5)
%%
%% Generator constraint: each field name is bound to a single strategy
%% (matching real-world namespace config). Generators never produce
%% mixed-strategy events for the same field. Strategy mismatch is its
%% own crash test (see EUnit suite).
%% =============================================================================

-module(bondy_oplog_fold_map_of_fields_proper_test).
...
inner_event_gen(strict_register) ->
    oneof([
        {set, hlc_gen(), value_gen()},
        {revoke, hlc_gen()},
        {resolve, hlc_gen(), value_gen()}
    ]);
inner_event_gen(ttl_presence) ->
    oneof([
        {issue, hlc_gen(), hlc_gen(), value_gen()},
        {revoke, hlc_gen()}
    ]).
