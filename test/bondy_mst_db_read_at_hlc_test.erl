%% =============================================================================
%% Tests for `bondy_mst_db:read_at_hlc/3` (`MST_DB_DESIGN.md` §10, wired
%% in D6).
%%
...
-module(bondy_mst_db_read_at_hlc_test).
...
unknown_namespace_returns_no_shards() ->
    ...
    ?assertEqual(
        {error, no_shards},
        bondy_mst_db:read_at_hlc(NS, <<"k">>, 100)
    ).
...
absent_cell_returns_initial_value_at_zero() ->
    ...
    ?assertEqual({ok, undefined, 0},
                 bondy_mst_db:read_at_hlc(NS, <<"absent">>, 100)),
...
projection_at_or_before_t_returns_projection() ->
    ...
    ?assertEqual({ok, {set, <<"v">>, 5}, 5},
                 bondy_mst_db:read_at_hlc(NS, <<"k">>, 5)),
    teardown_shard(Setup).
...
overlay_events_through_t_are_folded() ->
    ...
    ?assertEqual({ok, {set, <<"mid">>, 10}, 10},
                 bondy_mst_db:read_at_hlc(NS, <<"k">>, 15)),
    teardown_shard(Setup).
...
mix_projection_and_overlay_with_partial_window() ->
    ...
    ?assertEqual({ok, {set, <<"v10">>, 10}, 10},
                 bondy_mst_db:read_at_hlc(NS, <<"k">>, 12)),
    teardown_shard(Setup).
