...
field_event_is_idempotent_test() ->
    ...
    S1 = apply_ev(?MOD:initial_value(), E),
    S2 = apply_ev(S1, E),
    ?assertEqual(S1, S2).
...
merge_same_field_same_strategy_test() ->
    ...
    B = apply_ev(#{}, {field_event, <<"x">>, lww_register, {set, H2, <<"b">>}}),
    Merged = ?MOD:merge_states(A, B),
...
dispatcher_apply_event_via_shorthand_test() ->
    H = hlc(1, 0),
    E = {field_event, <<"x">>, lww_register, {set, H, <<"v">>}},
    {S, Delta} = bondy_oplog_fold:apply_event(map_of_fields, #{}, E, undefined),
    ?assertMatch(#{<<"x">> := {lww_register, _}}, S),
    ?assertEqual({field, <<"x">>, lww_register, <<"v">>}, Delta).
