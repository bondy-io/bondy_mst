...
secondary_shards_registered({_Db, Table, _Sup, _Dir}) ->
    ...
    lists:foreach(
        fun(Shard) ->
            ...
            ?assertEqual(
                4, bondy_db_core_registry:entry_shard_count(Entry)
            )
        end,
        lists:seq(0, 3)
    ),
    ?assertEqual(not_found, bondy_db_core_registry:lookup(NS, by_status, 4)).

custom_sec_shard_count({_Db, Table, _Sup, _Dir}) ->
    NS = maps:get(namespace, bondy_db:info(Table)),
    %% by_age declared sec_shard_count => 2.
    ?assertMatch({ok, _}, bondy_db_core_registry:lookup(NS, by_age, 0)),
    ?assertMatch({ok, _}, bondy_db_core_registry:lookup(NS, by_age, 1)),
    ?assertEqual(not_found, bondy_db_core_registry:lookup(NS, by_age, 2)),
    ?assertEqual({ok, 2}, bondy_db_core_registry:shard_count(NS, by_age)).
...
max_lag_refusal({_Db, Table, _Sup, _Dir}) ->
    ...
    lists:foreach(
        fun(S) ->
            {ok, E} = bondy_db_core_registry:lookup(NS, by_status, S),
            ok = bondy_db_core_registry:index_mark_rebuild(E)
        end,
        lists:seq(0, N - 1)
    ),
    ?assertMatch(
...
teardown_unregisters_shards({_Db, Table, _Sup, _Dir}) ->
    NS = maps:get(namespace, bondy_db:info(Table)),
    ?assertMatch({ok, _}, bondy_db_core_registry:lookup(NS, by_status, 0)),
