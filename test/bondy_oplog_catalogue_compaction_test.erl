...
truncates_and_preserves_reads() ->
    ...
    try
        ...
        ?assertEqual(
            {<<"v-a">>, 10}, bondy_db_core:read(NS, primary, <<"a">>)
        ),
        ?assertEqual(
            {<<"v-c">>, 30}, bondy_db_core:read(NS, primary, <<"c">>)
        ),
        ...
        ?assertEqual(
            {<<"v-b">>, 20}, bondy_db_core:read(NS, primary, <<"b">>)
        ),
        ?assertEqual(
            {<<"v-a">>, 10}, bondy_db_core:read(NS, primary, <<"a">>)
        ),
        ...
        _ = bondy_oplog_instance:await_apply(InstId),
        ?assertEqual(
            {<<"v-d">>, 40}, bondy_db_core:read(NS, primary, <<"d">>)
        )
    after
...
crdt_kernel_compaction_matches_from_scratch() ->
    ...
    try
        ...
        lists:foreach(
            fun({K, Want}) ->
                ARead = bondy_db_core:read(ANS, primary, K),
                BRead = bondy_db_core:read(BNS, primary, K),
