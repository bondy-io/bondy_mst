...
%%
%% Pins the key shape `{CellKey, EventHlc, EventKey}`, the HLC-windowed
%% select helpers, the range scan, and the post-projection eviction
%% match-spec.
%% =============================================================================
...
insert_single_event_test() ->
    ...
    ok = bondy_oplog_db_overlay:insert(Tab, <<"k">>, E),
...
events_for_returns_events_above_hlc_test() ->
    ...
    [bondy_oplog_db_overlay:insert(Tab, <<"k">>, E) || E <- [E1, E2, E3]],
    ?assertEqual(
        [E2, E3],
        bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 5)
    ),
    ?assertEqual(
        [E1, E2, E3],
        bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 0)
    ),
    ?assertEqual(
        [],
        bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 15)
    ),
    ok = bondy_oplog_db_overlay:delete(Tab).
...
events_for_isolates_by_cell_key_test() ->
    ...
    bondy_oplog_db_overlay:insert(Tab, <<"a">>, Ea),
    bondy_oplog_db_overlay:insert(Tab, <<"b">>, Eb),
    ?assertEqual([Ea], bondy_oplog_db_overlay:events_for(Tab, <<"a">>, 0)),
    ?assertEqual([Eb], bondy_oplog_db_overlay:events_for(Tab, <<"b">>, 0)),
    ok = bondy_oplog_db_overlay:delete(Tab).
...
events_for_orders_by_hlc_then_origin_then_seq_test() ->
    ...
    bondy_oplog_db_overlay:insert(Tab, <<"k">>, E_b),
    bondy_oplog_db_overlay:insert(Tab, <<"k">>, E_a),
    ?assertEqual(
        [E_a, E_b],
        bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 0)
    ),
...
range_returns_keys_in_window_test() ->
    ...
    Eb = mk_event(<<"b">>, 5, <<"o">>, 1),
    Ec = mk_event(<<"c">>, 5, <<"o">>, 2),
    [bondy_oplog_db_overlay:insert(Tab, K, E)
     || {K, E} <- [{<<"a">>, Ea}, {<<"b">>, Eb}, {<<"c">>, Ec}]],
    %% Half-open interval [a, c) — excludes c.
    ?assertEqual(
        [{<<"a">>, Ea}, {<<"b">>, Eb}],
        bondy_oplog_db_overlay:range(Tab, <<"a">>, <<"c">>, 0)
    ),
    ok = bondy_oplog_db_overlay:delete(Tab).
...
range_filters_by_hlc_window_test() ->
    ...
    ?assertEqual(
        [{<<"b">>, Eb}],
        bondy_oplog_db_overlay:range(Tab, <<"a">>, <<"z">>, 10)
    ),
    ok = bondy_oplog_db_overlay:delete(Tab).
...
evict_to_removes_rows_at_or_below_watermark_test() ->
    ...
    ?assertEqual([E3], bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 0)),
    ok = bondy_oplog_db_overlay:delete(Tab).
...
evict_to_preserves_higher_hlc_at_same_cell_test() ->
    ...
    bondy_oplog_db_overlay:insert(Tab, <<"k">>, E1),
    bondy_oplog_db_overlay:insert(Tab, <<"k">>, E2),
    Deleted = bondy_oplog_db_overlay:evict_to(Tab, 5, bondy_oplog_event:key(E1)),
    ?assertEqual(1, Deleted),
    ?assertEqual([E2], bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 0)),
    ok = bondy_oplog_db_overlay:delete(Tab).
...
evict_to_uses_event_key_tiebreaker_at_same_hlc_test() ->
    ...
    bondy_oplog_db_overlay:insert(Tab, <<"k">>, Ea),
    bondy_oplog_db_overlay:insert(Tab, <<"k">>, Eb),
    ...
    ?assertEqual(1, Deleted),
    ?assertEqual([Eb], bondy_oplog_db_overlay:events_for(Tab, <<"k">>, 0)),
    ok = bondy_oplog_db_overlay:delete(Tab).
