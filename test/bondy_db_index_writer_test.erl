...
writer_pid_registered({Db, _Sup, _Dir}) ->
    ...
    lists:foreach(
        fun(Shard) ->
            {ok, Entry} = bondy_db_core_registry:lookup(NS, by_value, Shard),
            Pid = bondy_db_core_registry:entry_writer_pid(Entry),
            ?assert(is_pid(Pid)),
...
flush_index(Table, IndexName) ->
    ...
    lists:foreach(
        fun(Shard) ->
            {ok, Entry} = bondy_db_core_registry:lookup(NS, IndexName, Shard),
            Pid = bondy_db_core_registry:entry_writer_pid(Entry),
            true = is_pid(Pid),
