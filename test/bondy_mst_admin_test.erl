...
populate_tree(Root) ->
    ...
    write_file(filename:join([Root, "sub", "b.bin"]),
               <<"world", 0:8000/unit:8>>),
    write_file(filename:join([Root, "sub", "deeper", "c.txt"]),
               <<"deeper">>),
    ok.
...
restore_round_trip() ->
    ...
    after
        rmrf(Src), rmrf(Dst), rmrf(Restored)
    end.
...
roundtrip_with_live_instance() ->
    ...
    try
        ...
        ok = filelib:ensure_path(Storage),
        ?assertMatch({ok, _}, bondy_mst_admin:restore(Backup, Storage,
