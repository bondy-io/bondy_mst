%% =============================================================================
%% EUnit smoke tests for `bondy_oplog_fold_max_register`.
%%
%% Covers value-lattice max-merge, HLC max-merge (independent of value
%% lattice), encode round-trip, hlc/gc_threshold corners. CAI invariants
%% live in the proper test.
%% =============================================================================

-module(bondy_oplog_fold_max_register_test).

-include_lib("eunit/include/eunit.hrl").

-define(MOD, bondy_oplog_fold_max_register).
...
%% Extract the new state from `apply_event/3`, discarding the
%% value-delta; tests below assert on state shape.
apply_ev(State, Event, Meta) ->
    {NewState, _Delta} =
        bondy_oplog_fold_max_register:apply_event(State, Event, Meta),
    NewState.
...
higher_value_supersedes_test() ->
    ...
    H2 = hlc(200, 0),
    S0 = {5, H1},
    ?assertEqual({9, H2}, apply_ev(S0, {set, 9}, key(H2))).

lower_value_is_max_noop_but_hlc_advances_test() ->
    %% HLC always max — lattice registers track last-modified separately
    %% from the value lattice.
    H1 = hlc(100, 0),
    H2 = hlc(200, 0),
    S0 = {9, H1},
    ?assertEqual({9, H2}, apply_ev(S0, {set, 5}, key(H2))).
...
merge_takes_max_value_and_max_hlc_test() ->
    A = {5, hlc(200, 0)},
    B = {9, hlc(100, 0)},
    %% Value max = 9 (from B); HLC max = 200 (from A).
    ?assertEqual({9, hlc(200, 0)}, ?MOD:merge_states(A, B)),
    ?assertEqual({9, hlc(200, 0)}, ?MOD:merge_states(B, A)).
...
dispatcher_resolves_shorthand_test() ->
    ?assertEqual(?MOD, bondy_oplog_fold:mod_of(max_register)).

dispatcher_is_known_test() ->
    ?assertEqual(true, bondy_oplog_fold:is_known(max_register)).
