%% =============================================================================
%% PropEr properties for `bondy_oplog_fold_lww_register`.
%%
...
-module(bondy_oplog_fold_lww_register_proper_test).
...
%% Small, opaque payload; binary() so encode/decode round-trip is well
%% defined. Limited size keeps shrunk counter-examples readable.
value_gen() ->
...
%% Unlike `presence_basic`, LWW has no terminal state and concurrent
%% writers are part of its model — we don't need to constrain the
%% generator. The fold's tie-break makes every sequence well-defined.
events_gen() ->
    list(event_gen()).
...
%% §5.3 — merge_states is idempotent: merge(A, A) == A.
