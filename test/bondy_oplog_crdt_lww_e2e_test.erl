...
ov_highest_hlc_wins(#{ns := NS}) ->
    ...
    ?assertEqual(
        {<<"newest">>, 20}, bondy_db_core:read(NS, primary, ?B, <<"k1">>)
    ).

ov_clear_clears(#{ns := NS}) ->
    materialise(NS, <<"k2">>, {set, <<"v">>, 5}, 5),
    overlay_insert(NS, <<"k2">>, 20, {clear, 20}),
    ?assertEqual(undefined, bondy_db_core:read(NS, primary, ?B, <<"k2">>)).
...
ov_below_projection_ignored(#{ns := NS}) ->
    ...
    ?assertEqual(
        {<<"current">>, 50}, bondy_db_core:read(NS, primary, ?B, <<"k3">>)
    ).
...
crdt_read_round_trips() ->
    ...
    _ = barrier(Id),
    ?assertEqual({<<"v">>, 42}, bondy_db_core:read(NS, primary, <<"bob">>)),
    teardown_instance(Id, NS, Cache, Proj).
...
clear_then_resurrect() ->
    ...
    _ = barrier(Id),
    ?assertEqual({<<"v2">>, 3}, bondy_db_core:read(NS, primary, <<"k">>)),
    teardown_instance(Id, NS, Cache, Proj).

registry_records_crdt_module() ->
    {Id, NS, Cache, Proj} = setup_instance(),
    {ok, Entry} = bondy_db_core_registry:lookup(NS, primary, 0),
    ?assertEqual(?CRDT, bondy_db_core_registry:entry_crdt_module(Entry)),
    teardown_instance(Id, NS, Cache, Proj).
